...
| 5 | שמירה — טלפון + פרטי הורה + סיסמה |
| 6 | הלינק שלך — `העתקה` · `שיתוף בוואטסאפ` · "הוסיפי למסך הבית" |

מסך 4 הוא הציר. עד אליו לא ביקשנו כלום.
בהרשמה: מייל להורה עם לינק לסגירת החנות. זו הבקרה ההורית היחידה.

**חזרה לניהול:** PWA למסך הבית · כפתור צף "עריכה" בחנות כשמחוברים · שחזור סיסמה לאימייל ההורה.
...
- `parent_phone` חובה. לא מוצג בחנות
