...
-- ═══════════════════════════════════════════════════════════════
--  ננה: שחרור מלא + בדיקה
--  רץ אחרי כל המיגרציות שלמעלה, ולכן sms_unlimited כבר קיים.
-- ═══════════════════════════════════════════════════════════════

-- 1. החנות פעילה ולא חסומה, והלינק פתוח להזמנות
update stores
   set status       = 'active',
       activated_at = coalesce(activated_at, now())
 where contact_phone = '972505278685';

-- 2. פטור קבוע ממכסת הסמס
update stores
   set sms_unlimited = true
 where contact_phone = '972505278685';

-- 3. איפוס המכסה של היום, כדי שתוכל להיכנס עכשיו
delete from phone_otps where phone = '972505278685';

-- 4. בדיקה. שלוש השורות אמורות לחזור תקינות:
select display_name                      as "החנות",
       status                            as "סטטוס (צריך active)",
       (activated_at is not null)        as "מפורסמת",
       sms_unlimited                     as "סמס ללא הגבלה",
       (select count(*) from products p
         where p.store_id = s.id and p.deleted_at is null) as "מוצרים",
       (select count(*) from phone_otps
         where phone = s.contact_phone)  as "בקשות סמס פתוחות (צריך 0)"
  from stores s
 where contact_phone = '972505278685';

-- 5. אין שום מגבלת מוצרים או הזמנות בדאטהבייס. אימות:
select count(*) as "טריגרים חוסמים על products/orders (צריך 0)"
  from information_schema.triggers
 where event_object_table in ('products','orders');
