...
`supabase/tests/local-stack/` — סביבה שמדמה את Supabase ו-R2 מקומית: PostgREST אמיתי (בינארי) מול Postgres, `supashim.mjs` (auth תואם-GoTrue: signup / login / user / admin), `s3shim.mjs` (אחסון עם presigned PUT), ו-`e2e.mjs` — תסריט Playwright של **29 בדיקות** שמריץ את כל לולאת המוצר:
