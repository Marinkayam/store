...
3. **Authentication → Sign In / Up → כבי "Confirm email".** ההרשמה קורית בסוף האונבורדינג, אחרי שהילדה כבר בנתה חנות — מסך אימות באמצע שובר את הפלואו. (האימייל של ההורה הוא חשבון ההתחברות עצמו — זו הבקרה ההורית של v1.)
...
נרשמים ל-[sms4free.co.il](https://www.sms4free.co.il) (חינם, מכסה יומית קטנה — מספיקה לקצב פתיחת חנויות) וממלאים את `SMS4FREE_*` ב-env. בפתיחת כל חנות נשלח SMS לטלפון של ההורה עם לינק לחנות ולינק **ניהול הורה** (`/p/{token}`) שמאפשר להשהות ולהפעיל את החנות בלי חשבון. ככה ההורה בלופ גם אם האימייל שהוזן בהרשמה הוא של הילדה. בלי ההגדרות האלה המערכת עובדת רגיל, פשוט בלי SMS.

### 6. קרון יומי (חובה בתוכנית החינמית)
...
- מכסות: 20 מוצרים · 25MB מדיה לחנות · 5 הזמנות מ-IP ליום · 3 חנויות לאימייל הורה
- מחיקת מוצרים היא תמיד soft delete (`deleted_at`)
- החנות הפומבית נקראת בשרת בלבד עם service role ושדות מפורשים — `contact_phone` ו-`parent_email` לא דולפים
